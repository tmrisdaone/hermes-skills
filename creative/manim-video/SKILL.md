...
\n\n## Production Pitfalls\n\n- **Termux/Android System Dependencies:** Manim CE requires `pangocairo`, `xproto`, and `xrender`. These often fail to build via `pip` in Termux due to missing system headers.\n- **Fallback Rendering:** If Manim installation fails, use the **PIL + FFmpeg sequence** pattern. Generate frames as JPEGs/PNGs using `PIL.ImageDraw` and stitch them via `ffmpeg -framerate 30 -i frame_%03d.jpg`. This provides a reliable fallback for motion graphics on Android without needing a full LaTeX/Cairo environment.

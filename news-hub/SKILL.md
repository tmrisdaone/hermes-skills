...
4. **Initiate Searches**:
   ...
   * **Fallback**: If `web_search`, `web_extract`, or `browser_navigate` are unavailable (API errors, missing backends, no browser running), use `terminal` + `curl` to hit public APIs (HN Algolia) and RSS feeds directly. See `references/fallback-news-sources.md` for exact commands, parsing snippets, and source rankings.
